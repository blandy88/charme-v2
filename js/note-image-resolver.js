...
(function installSmoothTopNavHideOnScroll() {
  if (typeof window === "undefined") return;
  if (window.__charmeNavHideFixInstalled) return;
  window.__charmeNavHideFixInstalled = true;

  function boot() {
    if (window.__charmeNavHideFixBooted) return;
    const body = document.body;
    const navbar = document.querySelector(".navbar");
    if (!body || !navbar) return;

    window.__charmeNavHideFixBooted = true;
    ...
    const chromeElements = [navbar, marquee].filter(Boolean);
    ...
    function marqueeVisible() {
      return Boolean(
        marquee &&
        body.classList.contains("has-marquee") &&
        getComputedStyle(marquee).display !== "none"
      );
    }

    function chromeFocused() {
      const active = document.activeElement;
      return Boolean(active && chromeElements.some((el) => el && el.contains(active)));
    }

    function chromeLockedOpen() {
      return chromeFocused() || Boolean(document.querySelector(openSurfaceSelector));
    }

    function setTransitions() {
      navbar.style.willChange = "transform, opacity";
      navbar.style.transition = "transform 320ms cubic-bezier(0.22, 1, 0.36, 1), opacity 220ms ease";
      if (marquee) {
        marquee.style.willChange = "transform, opacity";
        marquee.style.transition = "transform 280ms cubic-bezier(0.22, 1, 0.36, 1), opacity 180ms ease";
      }
    }
    ...
    function measure() {
      ...
      const visibleMarqueeHeight = marqueeVisible()
        ? (marquee.getBoundingClientRect().height || marquee.offsetHeight || 0)
        : 0;

      navShift = -Math.ceil(navTop + navHeight + 10);
      marqueeShift = -Math.ceil(visibleMarqueeHeight + 8);
    }

    function showChrome() {
      body.classList.remove("top-shell-hidden");
    ...
    function hideChrome() {
      body.classList.remove("top-shell-hidden");
      navbar.style.transform = `translateY(${navShift}px)`;
      navbar.style.opacity = "0.98";
      if (marquee && marqueeVisible()) {
        marquee.style.transform = `translateY(${marqueeShift}px)`;
    ...
    const bodyObserver = new MutationObserver(() => {
      if (body.classList.contains("top-shell-hidden")) {
        body.classList.remove("top-shell-hidden");
      }
      measure();
      requestUpdate();
